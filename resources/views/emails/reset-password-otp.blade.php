<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Kode OTP Reset Password</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2>Halo {{ $user->name }}!</h2>

        <p>Kami menerima permintaan untuk mereset password akun Anda. Masukkan kode verifikasi berikut untuk melanjutkan:</p>

        <div style="background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 6px; padding: 20px; text-align: center; margin: 20px 0;">
            <h1 style="color: #007bff; font-size: 36px; margin: 0; letter-spacing: 5px;">{{ $user->otp_login }}</h1>
        </div>

        <p>Kode ini hanya berlaku untuk satu kali penggunaan dan akan kadaluarsa dalam 10 menit.</p>

        <p>Jika Anda tidak meminta reset password, abaikan email ini dan password Anda akan tetap aman.</p>

        <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;">
        <p style="font-size: 12px; color: #666;">Email ini dikirim secara otomatis, mohon tidak membalas.</p>
    </div>
</body>
</html>
